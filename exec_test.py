...
with torch.no_grad(): 
    ...
    ax1.imshow(tr(outputs[0][0]),aspect='auto',cmap='viridis')
    ax2.set_title('Long hard true')                                                                               
    ax2.imshow(tr(data_.type(torch.FloatTensor)[0][0]),aspect='auto',cmap='viridis')
    plt.savefig('output_test_image_0')

    f, (ax1,ax2) = plt.subplots(2, 1, sharex=True, figsize=(8,8))                                  
    ax1.set_title('Mid hard reconstructed')                                                                               
    ax1.imshow(tr(outputs[0][4]),aspect='auto',cmap='viridis')
    ax2.set_title('Mid hard true')                                                                               
    ax2.imshow(tr(data_.type(torch.FloatTensor)[0][4]),aspect='auto',cmap='viridis')
    plt.savefig('output_test_image_1')

    f, (ax1,ax2) = plt.subplots(2, 1, sharex=True, figsize=(8,8))                                  
    ax1.set_title('Short hard reconstructed')                                                                               
    ax1.imshow(tr(outputs[0][8]),aspect='auto',cmap='viridis')
    ax2.set_title('Short hard true')                                                                               
    ax2.imshow(tr(data_.type(torch.FloatTensor)[0][8]),aspect='auto',cmap='viridis')
